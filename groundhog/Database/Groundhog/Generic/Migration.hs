...
data Reference = Reference {
  ...
  , referencedColumns :: [(Text, Text)] -- ^ child column, parent column
...
type QualifiedName = (Maybe Text, Text)

-- | Either column name or expression
type UniqueDefInfo = UniqueDef' Text (Either Text Text)
data TableInfo = TableInfo {
    tableColumns :: [Column]
  , tableUniques :: [UniqueDefInfo]
    -- | constraint name and reference
  , tableReferences :: [(Maybe Text, Reference)]
} deriving Show

data AlterColumn = Type DbTypePrimitive | IsNull | NotNull
                 | Default Text | NoDefault | UpdateValue Text deriving Show
...
data AlterDB = AddTable Text
             ...
             | AlterTable QualifiedName Text TableInfo TableInfo [AlterTable]
             ...
             | AddTriggerOnDelete QualifiedName QualifiedName Text
             -- | Qualified trigger name, qualified table name, field name, body
             | AddTriggerOnUpdate QualifiedName QualifiedName (Maybe Text) Text
             -- | Statement which creates the function
             | CreateOrReplaceFunction Text
             -- | Qualified function name
             | DropFunction QualifiedName
             -- | Schema name, if not exists
             | CreateSchema Text Bool
  deriving Show

data MigrationPack m = MigrationPack {
    compareTypes :: DbTypePrimitive -> DbTypePrimitive -> Bool
  , compareRefs :: (Maybe Text, Reference) -> (Maybe Text, Reference) -> Bool
  , compareUniqs :: UniqueDefInfo -> UniqueDefInfo -> Bool
  , compareDefaults :: Text -> Text -> Bool
  , migTriggerOnDelete :: QualifiedName -> [(Text, Text)] -> m (Bool, [AlterDB])
  , migTriggerOnUpdate :: QualifiedName -> [(Text, Text)] -> m [(Bool, [AlterDB])]
  ...
  , addUniquesReferences :: [UniqueDefInfo] -> [Reference] -> ([Text], [AlterTable])
  , showSqlType :: DbTypePrimitive -> Text
...
mkColumns :: DbTypePrimitive -> (Text, DbType) -> ([Column] -> [Column])
...
mkReferences :: DbTypePrimitive -> (Text, DbType) -> [Reference]
...
traverseDbType :: (DbType -> [Text] -> a) -> (Text, DbType) -> [a]
...
migrateSchema :: SchemaAnalyzer m => MigrationPack m -> Text -> m SingleMigration
        ...
        Just mainStructure' -> if null $ getAlters m mainStructure' expectedMainStructure
          then let
               ...
               updateDiscriminators = go 0 . map (head &&& length) . group . map fst $ res where
                  go acc ((False, n):(True, n2):xs) = (False, defaultPriority, "UPDATE " <> escape name <> " SET discr = discr + " <> (T.pack $ show n) <> " WHERE discr >= " <>  (T.pack $ show acc)) : go (acc + n + n2) xs
...
migrateList m@MigrationPack{..} (DbList mainName t) = do
  ...
  let valuesName = mainName <> delim <> "values"
      ...
      valuesQuery = "CREATE TABLE " <> escape valuesName <> " (" <> T.intercalate ", " (map showColumn valueColumns <> addInCreate) <> ")"
...
defaultMigConstr m@MigrationPack{..} e constr = do
  ...
  let (expectedTableStructure, (addTable, addInAlters)) = (case constrAutoKeyName constr of
        ...
        f autoKey cols uniqs refs' = (addTable', addInAlters') where
          ...
          addTable' = "CREATE TABLE " <> tableName escape e constr <> " (" <> T.intercalate ", " items <> ")"
...
class (Applicative m, Monad m) => SchemaAnalyzer m where
  schemaExists :: Text -- ^ Schema name
               -> m Bool
  getCurrentSchema :: m (Maybe Text)
  ...
  analyzeFunction :: QualifiedName -- ^ Qualified function name
                  -> m (Maybe (Maybe [DbTypePrimitive], Maybe DbTypePrimitive, Text)) -- ^ Argument types, return type, and body
